...
class MainMenuState extends MusicBeatState
{
	...
	override function create()
	{
		...
		var bg:FlxSprite = new FlxSprite(-80).loadGraphic(Paths.image((ClientPrefs.darkmode ? 'menuDesatDark' : 'menuDesat')));
		...
		magenta = new FlxSprite(-80).loadGraphic(Paths.image((ClientPrefs.darkmode ? 'menuDesatDark' : 'menuDesat')));
		...
		add(enterButton);
		debugButton = new VirtualButton(FlxG.width-125, FlxG.height-250, 'd');
		add(debugButton);
		#end
		FlxG.camera.follow(camFollowPos, null, 1);
		var versionShit:FlxText = new FlxText(12, FlxG.height - 56, 0, "Psych Engine v" + psychEngineVersion + "\nDragon Engine v" + Application.current.meta.get('version').split('-')[0] + (Application.current.meta.get('version').split('-')[1] != null && Application.current.meta.get('version').split('-')[1] != 'release' ? ('(' +Application.current.meta.get('version').split('-')[1].replace('.', ' ') + ')') : ''), 12);
