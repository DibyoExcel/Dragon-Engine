...
class FlxStringUtil
{
	 ...
	 */
	 ...
	 // i think only this i modified from build in flixel(unless has good idea to modify others too)
	 //lmao how many i comment for jokings(i just bored and make coding more fun lol)
	 //imagine got hired from flixel team and they see this code with all these comments lmao
	 //fantasicode lol
	 //this best way as solo developer to make coding fun lol
	 //*yawn*
	 //DAMN WHY I DOING THIS AT 2AM(joke i never coding at 2am)
	...
	public static function formatTime(Seconds:Float, ShowMS:Bool = false):String
	{
		...
		//this is look ridiculous ngl
		//but ok i guess some people might need it so here you go
		//also imagine 9999 weeks in a game timer lmao(technically possible but who would actually do that and i dont think float can handle that well)
		//end line jokes

		//1 week = 7 days = 168 hours = 10080 minutes = 604800 seconds
		//9999 weeks = 69993 days = 1679824 hours = 100789440 minutes = 6047664640 seconds(i guests float can still handle this number if 64-bit float is used if i remember correctly)
        if (week > 0) {
            timeString += week + ":";
        }
        if (day > 0) {
