...
class PlayState extends MusicBeatState
{
	...
	public static var ratingStuffDraconic:Array<Dynamic> = [
		['Scorched', 0.2], //From 0% to 19%
		['Dragon Droppings', 0.4], //From 20% to 39%
		['Cursed', 0.5], //From 40% to 49%
		['Hatchling', 0.6], //From 50% to 59%
		['Smolder', 0.69], //From 60% to 68%
		['Gleaming', 0.7], //69%
	...
	public var comboGroup:FlxTypedGroup<ComboSpr>;
	...
	public var defaultCamZoomHUD:Float = 1;
	...
	public static var storyLength:Float = 0;
	...
	//game textures(can use for lua too recommended change in `onCreate()`)
	//combo texture
	public var comboTexture:String = 'comboAtlas';
	public var numTexture:String = 'comboAtlas';
	//countdown texture
	public var countdownTexture:Array<String> = ['ready', 'set', 'go'];
	public var countdownTexturePixel:Array<String> = ['pixelUI/ready-pixel', 'pixelUI/set-pixel', 'pixelUI/date-pixel'];
	...
	override public function create()
	{
		...
		add(iconP2);
		iconP3 = new HealthIcon((gf != null ? gf.healthIcon : 'gf')/**default icon**/, false);
		...
		comboGroup = new FlxTypedGroup<ComboSpr>();
		...
		ratingGroup.cameras = [camHUD];
		comboGroup.cameras = [camHUD];
		numRatingGroup.cameras = [camHUD];
		...
		var luaGamemode:String = Paths.modFolders("gamemode/" + gamemode + ".lua");
	...
	function cacheCountdown()
	{
		...
		var func = CoolUtil.arrayFallback;
		introAssets.set('default', func(countdownTexture, ['ready', 'set', 'go']));
		introAssets.set('pixel', func(countdownTexturePixel, ['pixelUI/ready-pixel', 'pixelUI/set-pixel', 'pixelUI/date-pixel']));
			...
			startTimer = new FlxTimer().start(Conductor.crochet / 1000 / playbackRate, function(tmr:FlxTimer)
			{
				...
				switch (swagCounter)
				{
					...
					case 1:
						...
						insert(members.indexOf(ratingGroup), countdownReady);
					...
					case 2:
						...
						insert(members.indexOf(ratingGroup), countdownSet);
					...
					case 3:
						...
						insert(members.indexOf(ratingGroup), countdownGo);
	...
	{
		scoreTxt.text = (ClientPrefs.dragonW ? 'Coins: ' : 'Score: ') + songScore + (ClientPrefs.dragonW ? "K" : "")
		+ ' | ' + (ClientPrefs.dragonW ? 'Slay: ' : 'Misses: ') + songMisses
		+ ' | ' + (ClientPrefs.dragonW ? 'Fang: ' : 'Rating: ') + ratingName
			...
			for (songNotes in section.sectionNotes)
			{
				...
				for (i in 0...multNote) {
					...
					if (modcharttype == 'random flip scroll') {
						swagNote.flipScroll = FlxG.random.bool(50);
					}
					if (modcharttype == 'random direction scroll ') {
						swagNote.direction = FlxG.random.float(0, 360);
					}
	...
	function set_mergeHealthColor(value:Bool):Bool {
		if (mergeHealthColor != value) {
			...
			if (iconP3 != null) iconP3.visible = value;
			reloadHealthBarColors();
		}
		return value;
	}
	...
	static function degToRad(deg:Int) {
		//why haxe this not exits
	...
	override public function update(elapsed:Float)
	{
		...
		if (modcharttype == "wave note") {
			for (i in notes) {
				var dy = i.distance;
				var sus = i.isSustainNote;
				i.offsetX = (Math.sin(dy*0.01)*100)+(sus ? 36 : 0);
				if (sus) {
					i.copyAngle = true;
				}
				i.offsetAngle = Math.sin((dy*0.01)-degToRad(90))*45;
			}
		}
		if (modcharttype == 'fade note') {
			for (i in notes) {
				var dy = i.distance;
				var dyabs = Math.abs(dy);
				if (dyabs < 400 && dyabs >= 250) {
					i.multAlpha = (dyabs-350)/150;
				}
			}
		}
		switch (curStage)
		...
		{
			// why ( ͡° ͜ʖ ͡°)
			if(gf != null && gf.animation.curAnim != null && gf.holdTimer > Conductor.stepCrochet * (0.0011 / FlxG.sound.music.pitch) * gf.singDuration && gf.animation.curAnim.name.startsWith('sing')) {
				gf.dance();
			}
			if(!cpuControlled) {
				keyShit();
				...
				if(boyfriend.animation.curAnim != null && boyfriend != null && boyfriend.holdTimer > Conductor.stepCrochet * (0.0011 / FlxG.sound.music.pitch) * boyfriend.singDuration && boyfriend.animation.curAnim.name.startsWith('sing') && !boyfriend.animation.curAnim.name.endsWith('miss')) {
					boyfriend.dance();
				}
				if(dad != null && dad.animation.curAnim != null && dad.holdTimer > Conductor.stepCrochet * (0.0011 / FlxG.sound.music.pitch) * dad.singDuration && dad.animation.curAnim.name.startsWith('sing') && !dad.animation.curAnim.name.endsWith('miss')) {
					dad.dance();
				}
				//boyfriend.animation.curAnim.finish();
			}
	...
	public function endSong():Void
	{
		...
		if(ret != FunkinLua.Function_Stop && !transitioning) {
			if (SONG.validScore)
			{
				...
				Highscore.saveScore(SONG.song + (ClientPrefs.gameplaySettings.get('gamemode') == 'none' ? '' : ClientPrefs.gameplaySettings.get('gamemode')), songScore, storyDifficulty, percent);
				#end
			}
			playbackRate = 1;
			...
			if (isStoryMode)
			{
				...
				if (storyPlaylist.length <= 0)
				{
					...
					if (!cpuControlled && ClientPrefs.useResultScr) {
						var actualState = new dge.states.ResultScreen(campaignScore, campaignMisses, null, campaignPercent/StoryMenuState.songListLength);
					...
					if(!ClientPrefs.getGameplaySetting('practice', false) && !ClientPrefs.getGameplaySetting('botplay', false)) {
						...
						if (SONG.validScore)
						{
							Highscore.saveWeekScore(WeekData.getWeekFileName() + (ClientPrefs.gameplaySettings.get('gamemode') == 'none' ? '' : ClientPrefs.gameplaySettings.get('gamemode')), campaignScore, storyDifficulty);
						}
			...
			else
			{
				...
				//WeekData.loadTheFirstEnabledMod();//why??!?
				cancelMusicFadeTween();
				if(FlxTransitionableState.skipNextTransIn) {
					CustomFadeTransition.nextCamera = null;
				}
	...
	{
		var comboTex = comboTexture;
		var numTex = numTexture;
		if (PlayState.isPixelStage)
		{
	...
	private function spawnRatingSprite(note:Note, image:String = 'shit') {
		...
		var numTex = numTexture;
		if (note != null) {
			if (note.comboTexture != null && note.comboTexture.length > 0) comboTex = note.comboTexture;
			if (note.numTexture != null && note.numTexture.length > 0) numTex = note.numTexture;
		}
		if (PlayState.isPixelStage)
		{
			comboTex = 'pixelUI/' + comboTex + '-pixel';
		...
		if (note != null) {
			...
			rating.setupRating(comboTex, image, 2);
			rating.screenCenter();
			...
			if (showCombo)
			{
				...
				var comboSpr:ComboSpr = comboGroup.recycle(ComboSpr);
				comboSpr.setupRating(comboTex, image);
			...
			if(showComboNum) {
				for (i in seperatedScore)
				{
					...
					numScore.setupRating(numTex, 'num' + Std.string(i), 1, (isPixelStage ? 0.825 : 0.5));//ignore stupid scale
	...
	private function customKeyPress(key:Int, keyCheck:Bool) {
		if (!cpuControlled && startedCountdown && !paused && key > -1 && (keyCheck || ClientPrefs.controllerMode))
		{
			if(!boyfriend.stunned && generatedMusic && !endingSong)
			{
				...
				notes.forEachAlive(function(daNote:Note)
				{
					if (strumsBlocked[daNote.noteData] != true && !daNote.isSustainNote && canHitNote(daNote))//when player play as opponent the player cant press ignore note(based opponent itself), you cant press autoPress notes,notes only can hit if attach strums and not has been locked
					{
	...
	private function keyShit():Void
	{
		...
		if (startedCountdown && !boyfriend.stunned && generatedMusic)
		{
			...
			{
				// hold note functions
				if (strumsBlocked[daNote.noteData] != true && daNote.isSustainNote && parsedHoldArray[daNote.noteData] && canHitNote(daNote)) {
					goodNoteHit(daNote);
				}
			});
			...
			if (parsedHoldArray.contains(true) && !endingSong) {
				...
				if (gamemode != 'opponent' && boyfriend.animation.curAnim != null && boyfriend.holdTimer > Conductor.stepCrochet * (0.0011 / FlxG.sound.music.pitch) * boyfriend.singDuration && boyfriend.animation.curAnim.name.startsWith('sing') && !boyfriend.animation.curAnim.name.endsWith('miss'))
				{
					boyfriend.dance();
					//boyfriend.animation.curAnim.finish();
				}
	...
	function noteMiss(daNote:Note):Void { //You didn't hit the key and let it go offscreen, also used by Hurt Notes
		...
		combo = 0;
		if (!daNote.mustPress || daNote.isDad) {
			if (healthdrain) {
		...
		if (!daNote.mustPress || daNote.isDad) char = dad;
		...
		callOnLuas(!daNote.mustPress  ? 'opponentNoteMiss' : 'noteMiss', [notes.members.indexOf(daNote), daNote.noteData, daNote.noteType, daNote.isSustainNote]);
		if ((!daNote.isSustainNote && daNote.holdCover != null)) {
			daNote.holdCover.kill();
			daNote.holdCover = null;
		}
	...
	function noteMissPress(direction:Int = 1):Void //You pressed a key when there was no notes to press for this key
	{
		...
		if (!boyfriend.stunned)
		{
			...
			if (combo > 5 && gf != null && gf.animOffsets.exists('sad') && char == boyfriend)//imagine if gf sada because dad be like it creepy me out
			{
				gf.playAnim('sad');
			}
			combo = 0;
		...
		callOnLuas((gamemode == 'opponent' ? 'opponentNoteMissPress' : 'noteMissPress'), [direction]);
	}
	...
	function opponentNoteHit(note:Note):Void
		{
			if (!note.hitByOpponent && !note.wasGoodHit) {
				...
				if (!note.noAnimation) {
					...
					var animSuffix = note.animSuffix;
					...
					if (note.gfNote) {
						if (gf != null) {
							gf.playAnim(animToPlay + animSuffix, true, false, 0, note.isSustainNote);
							gf.holdTimer = 0;
						}
					} else {
						if (!note.mustPress) {
							dad.playAnim(animToPlay + animSuffix, true, false, 0, note.isSustainNote);
							dad.holdTimer = 0;
						} else {
							boyfriend.playAnim(animToPlay + animSuffix, true, false, 0, note.isSustainNote);
					...
					if (note.noteType == 'Hey!') {
						if (note.mustPress) {
							...
							if (dad.animOffsets.exists('hey')) {
								dad.playAnim('hey', true, false, 0, note.isSustainNote);
								dad.specialAnim = true;
								dad.heyTimer = 0.6;
							}
						}
		...
		if(ret != FunkinLua.Function_Stop)
		{
			...
			else
			{
				...
				if (!ClientPrefs.dragonW) {
					...
					if(ratingPercent >= 1) {
						ratingName = ratingStuffDraconic[ratingStuffDraconic.length-1][0]; //Uses last string
					} else {
						for (i in 0...ratingStuffDraconic.length-1) {
							if(ratingPercent < ratingStuffDraconic[i][1]) {
							ratingName = ratingStuffDraconic[i][0];
							break;
							}
						}
					}
			...
			// Rating FC
			ratingFC = "";
			if (sicks > 0) ratingFC = (!ClientPrefs.dragonW ? "PFC" : "Draconic Fang");
			if (goods > 0) ratingFC = (!ClientPrefs.dragonW ? "GFC" : "Vampire Fang");
			if (bads > 0 || shits > 0) ratingFC = (!ClientPrefs.dragonW ? "FC" : "Serpent's Fang");
			if (songMisses > 0 && songMisses < 10) ratingFC = (!ClientPrefs.dragonW ? "SDCB" : "Wolf Fang");
			else if (songMisses >= 10) ratingFC = (!ClientPrefs.dragonW ? "Clear" : "Fang Dagger");
		}
	...
	public function createStrum(tag:String= '', data:Int = 4, camera:String = 'hud', sfX:Float = 0, sfY:Float = 0, downScroll:Null<Bool> = null, player:Bool = false, gf:Bool = false) {
		...
		if (tag != '' && !strumGroupMap.exists(tag) && !notesGroupMap.exists(tag) && !noteSplashGroupMap.exists(tag) && !holdCoverGroupMap.exists(tag)) {
			...
			if (strumTamp != null) {
				...
				for (i in 0...data) {
					var babyArrow:StrumNote = new StrumNote(((ClientPrefs.middleScroll || gamemode == "bothside" ? FlxG.width * strumPointMiddle : FlxG.width*strumPointOpponent)-((Note.swagWidth*(Math.min(FlxG.width, 960)/960))*2))+((Note.swagWidth*(Math.min(FlxG.width, 960)/960))*i), strumLine.y, i, (player ? 1 : 0), gf);
	...
	function bfIndex(name:String):String {
