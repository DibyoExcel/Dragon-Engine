...
class NoteOffsetState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		rating.frames = Paths.getSparrowAtlas('comboAtlas');
		rating.animation.addByPrefix('sick', 'sick', 24);
		rating.cameras = [camHUD];
		rating.setGraphicSize(Std.int(rating.width * 0.7));
		rating.updateHitbox();
		rating.animation.play('sick');
		...
		for (i in seperatedScore)
		{
			...
			numScore.frames = Paths.getSparrowAtlas('comboAtlas');
			numScore.animation.addByPrefix('num' + i, 'num' + i, 24);
			numScore.cameras = [camHUD];
			numScore.setGraphicSize(Std.int(numScore.width * 0.5));
			numScore.updateHitbox();
			numScore.animation.play('num' + i);
	...
	override public function update(elapsed:Float)
	{
		...
		if(controls.BACK #if android || FlxG.android.justPressed.BACK #end)
		{
			...
			CustomFadeTransition.nextCamera = camOther;
			MusicBeatState.switchState(new options.PsychOptionsState());
