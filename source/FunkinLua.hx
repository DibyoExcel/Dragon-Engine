...
class FunkinLua {
	...
	public function new(script:String) {
		...
		// shader shit
		Lua_helper.add_callback(lua, "initLuaShader", function(name:String, glslVersion:Int = #if mobile 100 #else 120 #end) {
		...
		Lua_helper.add_callback(lua, "reorderCamera", function(orderCam:Array<String>) {
			...
			var camObject:Array<FlxCamera> = [];
			for (i in orderCam) {
				var camObj:FlxCamera = cameraBetterFromString(i);
				if (camObj != null && FlxG.cameras.list.indexOf(camObj) != -1) {
					camObject.push(camObj);
				}
			}
			CameraZOrder.reorderCamera(camObject);
		});
	...
	function initLuaShader(name:String, ?glslVersion:Int = #if mobile 100 #else 120 #end)
	{
		if(!ClientPrefs.shaders) return false;
