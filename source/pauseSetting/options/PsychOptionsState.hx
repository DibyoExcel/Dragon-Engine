...
class PsychOptionsState extends MusicBeatState
{
	...
	function openSelectedSubstate(label:String) {
		switch(label) {
			...
			case 'Graphics':
				openSubState(new pauseSetting.options.GraphicsSettingsSubState());
			case 'Visuals and UI':
				openSubState(new pauseSetting.options.VisualsUISubState());
			case 'Gameplay':
				openSubState(new pauseSetting.options.GameplaySettingsSubState());
			case 'Adjust Delay and Combo':
				LoadingState.loadAndSwitchState(new pauseSetting.options.NoteOffsetState());
		}
	}
	...
	override function update(elapsed:Float) {
		...
		if (controls.BACK) {
			...
			LoadingState.loadAndSwitchState(new pauseSetting.MainOptionsState());
		}
